    ...
    <div className="w-full max-w-xs border-r border-slate-200 dark:border-slate-700">
      ...
      <div className="overflow-y-auto">
        {mails.map((mail) => (
          ...
          >
            <p
              className={`font-semibold ${!mail.read ? 'text-slate-900 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}
            >
              {mail.sender}
            </p>
            <p
              className={`text-sm truncate ${!mail.read ? 'text-slate-700 dark:text-slate-300' : 'text-slate-500 dark:text-slate-500'}`}
            >
              {mail.subject}
            </p>
          </div>

  ...
  return (
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-slate-200 dark:border-slate-700">
             <TableHead className="text-slate-900 dark:text-slate-100">Product Name</TableHead>
              ...
              <TableHead className="text-slate-900 dark:text-slate-100">Stock</TableHead>
          ...
          { 
            currentProducts.map((product) => (
                 ...
                 <TableCell className='text-slate-500 dark:text-slate-400'>{product.category}</TableCell>
                  <TableCell className='text-slate-500 dark:text-slate-400'>{product.price}</TableCell>
                <TableCell className='text-slate-500 dark:text-slate-400'>{product.stock}</TableCell>
                <TableCell>
                ...
                <TableCell>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant='ghost' className='w-8 h-8 p-0 hover:bg-slate-100 dark:hover:bg-slate-800'>
                      ...
                      <DropdownMenuItem onSelect={()=>openDeleteDialog(product.id)} className='text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20'><Trash2 className='mr-2 w-4 h-4'/>Delete</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </TableCell>
              </TableRow>
    ...
    {/* Pagination Controls */}
      <div className='px-6 py-6 border-t border-slate-200 dark:border-slate-700 flex justify-end items-center gap-4'>
        ...
        <Button variant='outline' onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={ currentPage===1} className="border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">Previous</Button>
        <Button variant='outline' onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} disabled={ currentPage===totalPages} className="border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">Next</Button>
            </div>
      </div>
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteProduct} className="bg-red-500 hover:bg-red-600 text-white">Continue</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
              </AlertDialog>
